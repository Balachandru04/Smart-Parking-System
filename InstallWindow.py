...
class InstallWindow(QWidget):
    def __init__(self):
        ...
        self.resize(400, 300)  # Increased height for better layout
        ...
        labels = [
            ...
            "Admin Username:",
            "Admin Password:",
            "No of Two Wheeler Space:",
            "No. of Four Wheeler Space:"
        ]
        self.inputs = []
        ...
        for label_text in labels:
            ...
            input_field.setStyleSheet("padding:5px;font-size:17px")
            if "Password" in label_text:
                input_field.setEchoMode(QLineEdit.Password)  # Hide password input
            self.inputs.append(input_field)
            layout.addWidget(input_field)

        self.input_db_name, self.input_db_username, self.input_db_password, \
        self.input_admin_username, self.input_admin_password, \
        self.input_two_wheeler, self.input_four_wheeler = self.inputs
    ...
    def showStepInfo(self):
        ...
        if self.input_two_wheeler.text() == "" or not self.input_two_wheeler.text().isdigit():
            self.error_label.setText("Please Enter Valid Two Wheeler Space")
            return

        if self.input_four_wheeler.text() == "" or not self.input_four_wheeler.text().isdigit():
            self.error_label.setText("Please Enter Valid Four Wheeler Space")
            return
        ...
        try:
            ...
            self.error_label.setText(f"Error saving configuration: {str(e)}")